...
import com.zgkxzx.modbus4And.requset.ModbusReqa;
import com.zgkxzx.modbus4And.requset.ModbusReqb;
import com.zgkxzx.modbus4And.requset.ModbusReqc;
...
public class ModbusTcpModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void connectToModbusMaster1(String ip, int port, int timeout, int retries, final Callback cb) {

        ModbusReqa.getInstance().setParam(new ModbusParam()
//               .setHost("192.168.10.4")
    ...
    @ReactMethod
    public void connectToModbusMaster2(String ip, int port, int timeout, int retries, final Callback cb) {

        ModbusReqb.getInstance().setParam(new ModbusParam()
//               .setHost("192.168.10.4")
                .setHost(ip)
                .setPort(port)
                .setEncapsulated(false)
    ...
    @ReactMethod
    public void connectToModbusMaster3(String ip, int port, int timeout, int retries, final Callback cb) {

        ModbusReqc.getInstance().setParam(new ModbusParam()
//               .setHost("192.168.10.4")
                .setHost(ip)
                .setPort(port)
                .setEncapsulated(false)
    ...
    @ReactMethod
    public void readInputRegisters1(int slaveid, int start, int len, final Callback cb) {

        ModbusReqa.getInstance().readInputRegisters(new OnRequestBack<short[]>() {
            @Override
            public void onSuccess(short[] data) {
    ...
    @ReactMethod
    public void readInputRegisters2(int slaveid, int start, int len, final Callback cb) {

        ModbusReqb.getInstance().readInputRegisters(new OnRequestBack<short[]>() {
            @Override
            public void onSuccess(short[] data) {
                Log.d(TAG, "readInputRegisters onSuccess " + Arrays.toString(data));
                cb.invoke(Arrays.toString(data), null);
    ...
    @ReactMethod
    public void readInputRegisters3(int slaveid, int start, int len, final Callback cb) {

        ModbusReqc.getInstance().readInputRegisters(new OnRequestBack<short[]>() {
            @Override
            public void onSuccess(short[] data) {
                Log.d(TAG, "readInputRegisters onSuccess " + Arrays.toString(data));
                cb.invoke(Arrays.toString(data), null);
    ...
    // Destroy connection
    @ReactMethod
    public void destroyConnection1(Callback cb) {
        ModbusReqa.getInstance().destory();

        cb.invoke("Connection destroyed");
    }

    @ReactMethod
    public void destroyConnection2(Callback cb) {
        ModbusReqb.getInstance().destory();

        cb.invoke("Connection destroyed");
    }

    @ReactMethod
    public void destroyConnection3(Callback cb) {
        ModbusReqc.getInstance().destory();
